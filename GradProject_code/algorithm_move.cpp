...
void place_macro::shifting(){
    ...
    for(HPWLiter = HPWLmap.rbegin(); HPWLiter != HPWLmap.rend();++HPWLiter){
        for(auto wireiter = HPWLiter->second.begin(); wireiter != HPWLiter->second.end();++wireiter){
            /*determine_the_boundary_macro(wireiter->first);
            ...
            for(int i=0;i<10;i++){
                ...
                for(bounditer=find_boundary_macro.begin();bounditer!=find_boundary_macro.end();++bounditer){
                    cout<<"macro: "<<bounditer->macro_name<<", direction: "<<bounditer->direction<<endl;
    ...
    for(miter;miter!=macrolist.end();++miter){
        if(miter->name==macro_name){
            ...
            else if(direction=="up")
            {
                ...
                total_shifting_distance = total_shifting_distance + distance;    
            }
...
double place_macro::movable_distance(string macro_name, string direction){
    ...
    /*if(DTC<=0.2)
        times = 16;
    else if (DTC>0.2&&DTC<=0.5)
        times = 21;
    else if (DTC>0.5&&DTC<=0.8)
        times = 26; 
    else
        times = 31;*/
    times = 201;
    ...
    if(direction=="left"){ 
        ...
        for(int offset=1;offset<times;offset++){
            int overlap=0;
...
void place_macro::determine_the_boundary_macro(string wire_name){
  ...
  for(wireiter=onewiremap.begin(); wireiter!=onewiremap.end(); ++wireiter)//從wiremap裡面找吃到的線
  {
   ...
   //cout<<"tempt_down_macro: "<<tempt_down_macro; 
   vector<Boundary>:: iterator fiter = find_boundary_macro.begin();
   for(fiter== find_boundary_macro.begin();fiter!=find_boundary_macro.end();++fiter)
   {
     //cout<<"vector size:"<<find_boundary_macro.size()<<"\n";
     //cout<<"direction: "<<fiter->direction<<" "<<"macro_name: "<<fiter->macro_name<<"\n";
   }
   //return find_boundary_macro;
